...
/// </summary>
public sealed class MultiViewExportServiceIntegrationTests
{
    ...
    private static void WriteFixtureFiles(TempWorkspace solution)
    {
        var proj2Csproj = Path.Combine(solution.Root, "Proj2", "Proj2.csproj");
        Directory.CreateDirectory(Path.GetDirectoryName(proj2Csproj)!);
        File.WriteAllText(proj2Csproj, "<Project Sdk=\"Microsoft.NET.Sdk\"></Project>");
        ...
        File.WriteAllText(proj2InternalOnly, """
            namespace Fixture.Lib;
        ...
        File.WriteAllText(proj3Csproj, "<Project Sdk=\"Microsoft.NET.Sdk\"></Project>");

        var proj3OnlyInternal = Path.Combine(solution.Root, "Proj3", "OnlyInternal.cs");
        File.WriteAllText(proj3OnlyInternal, """
        ...
        File.WriteAllText(sidecarJson, """{"fixture":true}""");
    }

    private static async Task VerifyFixtureOutputs(
        string exportRoot,
        string isolatedSolRoot,
        string mergedRoot,
        Mock<IPostExportTask> post,
        string solutionName)
    {
        Assert.True(File.Exists(Path.Combine(exportRoot, "readme.md")));
        var globalReadme = await File.ReadAllTextAsync(Path.Combine(exportRoot, "readme.md"), TestContext.Current.CancellationToken);
        ...
        var isolatedReadme = await File.ReadAllTextAsync(Path.Combine(isolatedSolRoot, "readme.md"), TestContext.Current.CancellationToken);
        ...
        var signaturesMd = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "signatures-only", "FixtureSol.Proj1_signatures-only.md"), TestContext.Current.CancellationToken)
            + await File.ReadAllTextAsync(Path.Combine(mergedRoot, "signatures-only", "FixtureSol.Proj2_signatures-only.md"), TestContext.Current.CancellationToken)
            + await File.ReadAllTextAsync(Path.Combine(mergedRoot, "signatures-only", "FixtureSol.Proj3_signatures-only.md"), TestContext.Current.CancellationToken);
        ...
        var publicApi = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "public-only", "FixtureSol.Proj1_public-only.md"), TestContext.Current.CancellationToken);
        ...
        var publicProj2 = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "public-only", "FixtureSol.Proj2_public-only.md"), TestContext.Current.CancellationToken);
        ...
        var completeProj2 = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "complete", "FixtureSol.Proj2_complete.md"), TestContext.Current.CancellationToken);
        Assert.Contains("FixtureInternalOnlyMarker", completeProj2, StringComparison.Ordinal);

        var completeProj3 = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "complete", "FixtureSol.Proj3_complete.md"), TestContext.Current.CancellationToken);
        Assert.Contains("Proj3InternalOnlyType", completeProj3, StringComparison.Ordinal);

        var modelsMd = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "dto-only", "FixtureSol.Proj1_dto-only.md"), TestContext.Current.CancellationToken);
        ...
        var depGraph = await File.ReadAllTextAsync(Path.Combine(isolatedSolRoot, "dependency-graph.md"), TestContext.Current.CancellationToken);
        ...
        var fullSource = await File.ReadAllTextAsync(Path.Combine(mergedRoot, "complete", "FixtureSol.Proj1_complete.md"), TestContext.Current.CancellationToken);
    ...
    public async Task RunAsync_multi_view_tree_matches_konzept_and_content_samples()
    {
        ...
        fileDiscovery
            ...
            .Returns(ExtractionResult<List<string>>.Success([
                Path.Combine(solution.Root, "Proj1", "App.cs"),
                Path.Combine(solution.Root, "Proj1", "Models.cs"),
                Path.Combine(solution.Root, "Proj1", "sidecar.json")
            ]));
        fileDiscovery
            .Setup(f => f.FindFilesForProject(project2, It.IsAny<string>(), It.IsAny<AppSettings>()))
            .Returns(ExtractionResult<List<string>>.Success([
                Path.Combine(solution.Root, "Proj2", "LibTypes.cs"),
                Path.Combine(solution.Root, "Proj2", "FixtureInternalOnlyShell.cs")
            ]));
        fileDiscovery
            .Setup(f => f.FindFilesForProject(project3, It.IsAny<string>(), It.IsAny<AppSettings>()))
            .Returns(ExtractionResult<List<string>>.Success([
                Path.Combine(solution.Root, "Proj3", "OnlyInternal.cs")
            ]));
        ...
        await VerifyFixtureOutputs(export.Root, isolatedSolRoot, mergedRoot, post, solutionName);
    }
