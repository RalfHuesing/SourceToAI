    ...
    private static void PrepareSolutionExportRootDirectory(string outputDir)
    {
        ...
        if (Directory.Exists(outputDir) && !File.Exists(markerPath))
        {
            throw new SourceToAiValidationException(
                $"Sicherheitsabbruch: Das Zielverzeichnis '{outputDir}' existiert bereits, enthält aber keine Sicherheits-Markerdatei ({MultiViewExportPaths.SafetyMarkerFileName}). Um Datenverlust zu vermeiden, wurde die Operation abgebrochen. "
